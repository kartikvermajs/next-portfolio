...
export default function ProjectList({ repos }: { repos: any[] }) {
    ...
    return (
        <div ref={listRef} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {repos.map((repo) => (
                <div
                    key={repo.id}
                    className="project-card group relative bg-card border border-border p-6 rounded-2xl overflow-hidden hover:border-accent/40 transition-colors flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-accent/5"
                >

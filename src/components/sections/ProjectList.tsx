...
export default function ProjectList({ repos }: { repos: Project[] }) {
    ...
    return (
        <>
            <div ref={listRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                {repos.map((project) => (
                    <div
                        key={project.id}
                    ...
                    >
                        <div
                            className={`relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] ${flippedId === project.id ? "[transform:rotateY(180deg)]" : ""
                                }`}
                        >
                            {/* Front Face */}
                            <div className="absolute inset-0 w-full h-full bg-card border border-border rounded-2xl overflow-hidden [backface-visibility:hidden] shadow-sm hover:shadow-xl hover:shadow-accent/5 flex flex-col group">
                                ...
                                <div className="p-6 flex flex-col flex-grow z-10">
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-auto">
                                        {project.techStack.slice(0, 3).map((tech) => (
                                            <span key={tech} className="text-xs font-medium px-2.5 py-1 bg-border/50 text-muted-foreground rounded-md">
                                                {tech}
                                            </span>
                                        ))}
                            ...
                            {/* Back Face */}
                            <div className="absolute inset-0 w-full h-full bg-card border border-accent/40 rounded-2xl overflow-hidden [backface-visibility:hidden] [transform:rotateY(180deg)] shadow-xl flex flex-col justify-center items-center p-8">
                                ...
                                <div className="flex flex-col gap-4 w-full max-w-[200px]">
                                    <a
                                        ...
                                        className="flex items-center justify-center gap-3 w-full py-3 px-4 bg-secondary text-foreground rounded-lg hover:bg-secondary/80 transition-colors font-medium border border-border"
                                    >
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center justify-center gap-3 w-full py-3 px-4 bg-accent text-accent-foreground rounded-lg hover:bg-accent/90 transition-colors font-medium"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="flex items-center justify-center gap-3 w-full py-3 px-4 bg-transparent border-2 border-accent text-accent rounded-lg hover:bg-accent hover:text-accent-foreground transition-colors font-medium mt-2"
                                    >
                        ...
                        <div className="p-6 sm:p-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            <div className="flex flex-wrap gap-2 mb-6">
                                {selectedProject.techStack.map((tech) => (
                                    <span key={tech} className="text-xs font-semibold px-3 py-1 bg-accent/10 border border-accent/20 text-accent rounded-full">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                            ...
                            <div className="prose prose-sm sm:prose-base dark:prose-invert max-w-none">
                                {selectedProject.fullDescription.split('\n\n').map((paragraph, idx) => (
                                    <p key={idx} className="whitespace-pre-line text-muted-foreground leading-relaxed">
                                        {paragraph}
                                    </p>
                                ))}
                            </div>
